...
@pytest.mark.parametrize('data_type, data_ids, data_stats, exp1, exp2, exp3', [
...
def test_get_thing_data(data_type, data_ids, data_stats, exp1, exp2, exp3):
    url = 'http://www.boardgamegeek.com/xmlapi2'
    test_case = get_thing_data(data_type, data_ids)
...
@pytest.mark.parametrize('test, exp_id, exp_name, exp_votes, exp_votes2, exp_bayes', [
...
def test_loop_through_xml_tree(test, exp_id, exp_name, exp_votes, exp_votes2,
    ...
    assert test_case['items_item_id'] == [exp_id]
    assert test_case['item_name_value'] == [exp_name]
    assert test_case['results_result_numvotes'] == [exp_votes, exp_votes2]
    assert test_case['ranks_rank_bayesaverage'] == [exp_bayes]
